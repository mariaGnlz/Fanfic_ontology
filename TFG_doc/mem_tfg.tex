...
Volviendo al ejemplo de Romeo y Julieta, una frase como \textit{'I love you', said Romeo}, extraída de un texto más largo en la que el contexto es que Romeo está hablando con Julieta, da poca información a un algoritmo que no es capaz de entender a qué personajes se refieren los pronombres \textit{I} y \textit{you}. Sin embargo, si se aplicase resolución de correferencia sobre el texto, a ojos del algoritmo la frase se convertiría en \textit{'Romeo love Juliette', said Romeo}, con lo que el algoritmo entiende mucho mejor qué está pasando en esta frase y a quiénes afecta.
...
Mientras aprendía a manejar los objetos de \textit{Stanza}, me di cuenta de que los personajes más nombrados tienen, lógicamente, cadenas de correferencia más largas. Esto, combinado con el hecho de que los personajes protagonistas tienden a ser los más nombrados de una historia, hace que sea muy sencillo encontrar qué frases exactamente hacen referencia a los personajes protagonistas, incluso si sólo se les menciona de forma pronominal (mediante pronombres).

Se puede aprovechar esta información para acotar el alcance del problema: en las estrategias anteriores se buscaba asignar etiquetas a nivel de documento, como 'romance' o 'enemistad', sin embargo, pudiendo encontrar fácilmente las frases en las que se mencionan a los protagonistas y teniendo también la etiqueta de AO3, cuyo formato 'Personaje A/Personaje B' menciona explícitamente los personajes que aparecen en la relación, hace que se pueda reducir el problema de etiquetar documentos a etiquetar únicamente frases con menciones. 

Esta acotación funciona especialmente bien para el RFE dataset, que fue creado precisamente para capturar los relatos centrados alrededor de una única relación entre dos personajes. Dado un relato y extrayendo su etiqueta, el algoritmo debería responder a la pregunta '¿Tienen el personaje A y el personaje B una relación de tipo X?' y, para responderla, sólo tiene que tener en cuenta las frases que aparecen las cadenas de correferencia más largas del relato.
